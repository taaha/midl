...
# Get the token embeddings from Paligemma
def get_vocab_embeddings_paligemma(llm_model, tokenizer, device="cuda"):
...
def run_llava_model(
    ...
    input_ids = processor(text=text_prompt, images=images_tensor, return_tensors="pt")
...
def run_paligemma_model(
    ...
    model_inputs = processor(text=text_prompt, images=image, return_tensors="pt")
    ...
    with torch.inference_mode():
        generation = model.generate(**model_inputs, max_new_tokens=100, output_hidden_states=hidden_states, return_dict_in_generate=hidden_states, do_sample=False)
        # breakpoint()
...
def retrieve_logit_lens_paligemma(state, img_path, text_prompt = None):
    ...
    with torch.inference_mode():
        curr_layer_logits = state["model"].language_model.lm_head(hidden_states).cpu().float()
        # Move logits to CPU only after all GPU operations are done
        # curr_layer_logits = curr_layer_logits.cpu().float()
...
def get_caption_from_paligemma(
    img_path, model, processor, text_prompt=None
):
    image = Image.open(img_path)

    # Generate the new caption
    new_caption = run_paligemma_model(
        model,
        processor,
        text_prompt,
        image,
...
def load_paligemma_state(device="cuda"):
    ...
    # Get the embeddings for each token in vocabulary from Paligemma
    vocab_embeddings = get_vocab_embeddings_paligemma(model, tokenizer, device=device)


    execute_model = lambda img_path, text_prompt=None, image_embeddings=None: get_caption_from_paligemma(
        img_path, model, processor, text_prompt=text_prompt
    )
